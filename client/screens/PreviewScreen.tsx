...
import { PreviewCopy, CopyHelpers } from "@/constants/copy";
...
export default function PreviewScreen() {
  ...
  const [processingMessage, setProcessingMessage] = useState(PreviewCopy.processing.messages[0]);
    ...
    navigation.setOptions({
      headerLeft: () => (
        <HeaderButton
          ...
          pressOpacity={0.7}
        >
          <ThemedText style={styles.headerButton}>{PreviewCopy.retakeButton}</ThemedText>
        </HeaderButton>
      ),
    });
  }, [navigation]);

  // Rotate processing messages while processing
  React.useEffect(() => {
    if (isProcessing) {
      const interval = setInterval(() => {
        setProcessingMessage(CopyHelpers.randomProcessingMessage());
      }, 2500);
    ...
    try {
      ...
      if (err instanceof Error) {
        if (err.name === "AbortError") {
          errorMessage = PreviewCopy.errors.timeout;
        } else if (err.message.includes("Network request failed") || err.message.includes("TypeError")) {
          errorMessage = PreviewCopy.errors.network;
        } else {
          errorMessage = PreviewCopy.errors.failed(err.message);
        }
      }
        ...
        >
          {isProcessing ? (
            <View style={styles.loadingContent}>
              ...
              <ThemedText style={styles.loadingText}>{PreviewCopy.processing.button}</ThemedText>
            </View>
          ) : (
            PreviewCopy.generateButton
          )}
        </Button>
      </View>
